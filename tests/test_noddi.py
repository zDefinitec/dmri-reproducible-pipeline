...
def test_launcher_caps_workers_to_roi_voxels_and_uses_no_shell(tmp_path):
    ...
    def factory(argv, **kwargs):
        ...
        with h5py.File(
            ...
            handle.create_group("first999Exceptions")
...
def test_merge_requires_worker_exception_samples_in_metrics(tmp_path):
    context = _context(tmp_path, workers=2)
    _prepare_merge_fixture(context)
    metrics_path = context.stage_dir / "noddi_metrics.json"
    metrics = json.loads(metrics_path.read_text(encoding="utf-8"))
    del metrics["first_999_exceptions"]
    ...
    with pytest.raises(NODDIError, match="first_999_exceptions"):
        merge_noddi(context)


def test_merge_rejects_999_sample_that_does_not_match_worker_error_rows(tmp_path):
    context = _context(tmp_path, workers=2)
    _prepare_merge_fixture(context)
    with h5py.File(context.stage_dir / "NODDI_roi.mat", "r") as handle:
        indices = np.asarray(handle["idx"], dtype=int)
            ...
            "first_999_exceptions": [
                {
                    "worker": 1,
                    "global_row": 2,
                    "identifier": "fixture:WrongRow",
                    "message": "points at a successful row",
    ...
    with pytest.raises(NODDIError, match="first 999 rows"):
        merge_noddi(context)


def test_merge_rejects_zero_successful_voxels_with_partition_counts(tmp_path):
    context = _context(tmp_path, workers=2)
    _prepare_merge_fixture(context)
...
def test_real_matlab_worker_passes_column_signal_to_fit(tmp_path):
    ...
    (fitting / "ThreeStageFittingVoxel.m").write_text(
        ...
        "if ~isequal(size(signal),[2 1])\n"
        "error('fixture:SignalShape','signal must be a column');\n"
        "end\n"
    ...
    private.mkdir(parents=True)
    np.savetxt(stage / "bvals_rounded.txt", [[0, 1000]], fmt="%d")
    np.savetxt(
        stage / "eddy_rotated_bvecs.txt",
        [[0, 1], [0, 0], [0, 0]],
        fmt="%d",
    )
    ...
    expression = (
        f"root='{quote(stage)}';"
        "roi=[1 2];mask=uint32(1);idx=[1 1 1];"
    ...
    final_path = stage / "worker_01_final.mat"
    assert final_path.is_file()
    with h5py.File(final_path, "r") as handle:
        assert np.asarray(handle["error_code"]).reshape(-1).tolist() == [0]


def test_real_matlab_worker_exceptions_reach_merged_metrics(tmp_path):
    executable = _real_matlab()
    ...
    assert samples[0]["worker"] == 1
    assert samples[0]["global_row"] == 1
