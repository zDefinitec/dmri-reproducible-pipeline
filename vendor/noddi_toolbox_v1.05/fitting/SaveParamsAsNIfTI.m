...
% function SaveParamsAsNIfTI(paramsfile, roifile, targetfile, outputpref)
%
% This function converts the estimated model parameters stored in a mat
% file into the corresponding spatial maps in NIfTI format
%
%
% Input:
%
% paramsfile - the mat file storing the model parameters estimated with
%              batch_fitting_single or batch_fitting
%
% roifile    - the mat file storing the data used to estimate the model
%              parameters stored in paramsfile
%
% targetfile - the NIfTI file specifying the target 3-D volume formatting
%              information for the output parameter maps
%
% outputpref - a text string specifying the filename prefix of the output
%              parameter maps
%
%
% Output:      The parameter maps in NIfTI format; the nature and the
%              number of these depend on the model used to fit the data
%
%
% author: Gary Hui Zhang (gary.zhang@ucl.ac.uk)
%

% load the fitting parameter mat file
fprintf('loading the fitted parameters from : %s\n', paramsfile);
params = load(paramsfile);

% check if the fitting run has been interrupted or not
if isfield(params, 'current_index')
    % this must be an interrupted batch_fitting_single run
    fprintf('This is an interrupted batch_fitting_single run.\n');
    error('Please resume the run to its completion.');
elseif isfield(params, 'split_end')
    % this must be an interrupted batch_fitting run
    fprintf('This is an interrupted batch_fitting run.\n');
    error('Please resume the run to its completion.');
end

% if the fitting run is complete, proceed to load the fitting results
model = params.model;
mlps = params.mlps;
if (model.noOfStages == 3)
    mcmcps = params.mcmcps;
end
fobj_ml = params.fobj_ml;
